...
class ScaleUploadForm(FlaskForm):
    categoria = SelectField("Categoria", choices=[("Enfermagem","Enfermagem"),("Medica","M̩dica")], validators=[DataRequired()])
    servico = StringField("Servi̤o (ex: Cardiologia)", validators=[DataRequired(), Length(max=80)])
    ...
    mes = SelectField("M̻s", choices=[(str(i), str(i)) for i in range(1,13)], validators=[DataRequired()])
