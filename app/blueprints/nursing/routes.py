...
@login_required
def api_daily():
    """API para retornar dados da escala diária com lógica de status e contadores corrigida"""
    date_str = request.args.get('date')
    ...
    # 1. Buscar células da escala mensal planejada
    schedules = NursingMonthlySchedule.query.filter_by(
    ...
    for schedule in schedules:
        ...
        cells = NursingMonthlyCell.query.filter(
            NursingMonthlyCell.schedule_id == schedule.id,
            NursingMonthlyCell.day == target_date.day
        ).all()
        ...
        for cell in cells:
            ...
            # Busca se existe uma ação (override) da gerência para este usuário no dia
            ...
            planned_staff.append({
                ...
                "is_substitute": bool(cell.planned_user_id and cell.planned_user_id != cell.user_id),
            })

    # 2. Buscar profissionais extras/avulsos (Overrides sem célula mensal)
    overrides_without_cell = NursingDailyOverride.query.filter_by(date=target_date).all()
    for override in overrides_without_cell:
        if any(p["user_id"] == override.user_id for p in planned_staff):
            continue
        ...
        if sector_filter != 'all' and (not sector or str(sector.id) != str(sector_filter)):
            continue

        ov_shift = (override.shift or 'D').strip().upper()
        if shift_filter == 'diurno' and ov_shift != 'D': continue
        if shift_filter == 'noturno' and ov_shift != 'N': continue

        # Mapeamento de status para extras
        if override.code == "OK": status = "P"
        elif override.code in ("F", "FT"): status = "F"
        elif override.code == "AT": status = "AT"
        elif override.code == "COOP": status = "COOP"
        ...
        planned_staff.append({
            ...
            "user_id": user.id,
            "name": user.nome or f"Usuário {user.id}",
            "cargo": getattr(user, "role_label", user.role) or "Enfermagem",
            "sector_name": sector.name if sector else "Sem setor",
            "sector_id": sector.id if sector else None,
            "shift": ov_shift,
            "horario": "07:00 - 18:59" if ov_shift == 'D' else "19:00 - 06:59",
            "status": status,
            "tipo": "EXTRA",
    ...
    present = sum(1 for p in planned_staff if p["status"] == "P")
    coop = sum(1 for p in planned_staff if p["status"] == "COOP")
    absent = sum(1 for p in planned_staff if p["status"] == "F")
    atest = sum(1 for p in planned_staff if p["status"] == "AT")
    extra = sum(1 for p in planned_staff if p["status"] == "EXTRA")
    swap = sum(1 for p in planned_staff if p["status"] == "TROCA")
    
    # Pendentes: quem ainda está com o status padrão do sistema
    pending = sum(1 for p in planned_staff if p["status"] == "AGUARDANDO")

    planned_staff.sort(key=lambda x: ((x["sector_name"] or ""), (x["name"] or "")))

    return jsonify({
        "success": True,
        "daily_records": planned_staff,
        "stats": {
            "total": total,
            "present": present,
            "coop": coop,
...
def update_daily_status():
    ...
    if not user_id or not date_str or not code:
    ...
    if record:
        ...
        record = NursingDailyOverride(
            user_id=user_id, date=ref_date, code=code, shift=shift or "D"
        )
...
def add_professional():
    ...
    user_id = data.get("user_id")
    date_str = data.get("date")
    code = data.get("status") or "OK"
    shift = data.get("shift") or "D"
    ...
    if exists:
        return jsonify({"error": "Profissional já possui registro neste dia"}), 409

    record = NursingDailyOverride(
        user_id=user_id, date=ref_date, code=code, shift=shift,
